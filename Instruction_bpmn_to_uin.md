...
5. [Передача моделей бизнес-процессов в общий репозиторий с использованием Git](#слияние-моделей-бизнес-процессов-с-общим-репозиторием-с-использованием-git)
...
Также на этом шаге обязательно укажите свои данные введя в командную строку последовательно две команды. Сначала
...
```
 ...
 что соответственно обозначает вашу почту и имя, от которого вы будете затем осуществлять выгрузку в репозиторий, поэтому значения `you@example.com` и `Your Name` замените соответственно на вашу почту и имя пользователя.
...
Как работать с GitFlic вы можете подробно ознакомиться в [официальной документации](https://docs.gitflic.space/) платформы.

### Получение исходных данных и подготовка к работе с использованием Git

Для оформления процессов должен использоваться заранее подготовленный шаблон. Такой шаблон размещается на общем репозитории платформы GitFlic, ссылку на которую предоставит куратор проекта. В рамках же данной инструкции будет показан тестовый репозиторий с шаблоном для демонстрации принципа работы. Репозиторий называется BPMN_main и в нем будет лежать только один файл с именем `template.bpmn`.

![1697706792390](image/Instruction_bpmn_to_uin/1697706792390.png)

#### Создание копии репозитория (форка)

В дальнейшем в данном репозитории будут храниться все версии разработанных участниками моделей бизнес-процессов. GitFlic не является облачным сервисом, таким как Яндекс.Диск, Облако Mail.ru, и т.п. Для того, чтобы разработанные вами файлы оказались в общем пространстве, необходимо сделать копию данного репозитория (**форк**), в нем размещать свои разработки, а затем направлять запрос на слияние, который принимает администратор репозитория. Это делается для того, чтобы убедиться, что направляемые схемы корректны и могут быть использованы для дальнейшей обработки другими участниками.

Для того, чтобы сделать форк, данного репозитория в свой, необходимо:

1. Находясь в общем репозитории нажать на кнопку "Форк"
...
3. В результате откроется форк репозитория из перечня уже ваших проектов. Под названием форка также указана ссылка на родительский репозиторий (общий, из которого он был создан). В нем будет тот же самый файл шаблона, как и в общем репозитории.

![1697709639821](image/Instruction_bpmn_to_uin/1697709639821.png)

#### Клонирование репозитория

Следующим шагом необходимо сделать рабочую папку на локальном компьютере, которая будет связана с этим репозиторием. Для того, чтобы такая папка сформировалась, используется функция клонирования (git clone).

Для того, чтобы клонировать форк на локальный компьютер, необходимо сделать следующее:

1. Выбрать или создать рабочую папку, где будет храниться "клон" данного репозитория на компьютере.
...
3. Вызвать командную строку из папки, в которой будет располагаться клон репозитория. Для этого, например, можно зажав клавишу Shift и нажав правой кнопкой мыши, находясь в рабочей папке, выбрать команду контекстного меню "Открыть окно PowerShell здесь". В результате у вас откроется командная строка PowerShell с готовностью к выполнению команд внутри рабочей папки.
   ...
   ![1697719571479](image/Instruction_bpmn_to_uin/1697719571479.png)
4. Введите команду:
   ...
   Прежде чем произойдет клонирование, у вас появится окно "Git Credential Manager", в котором необходимо указать данные учетной записи, указанные при регистрации.

   ![1697719766061](image/Instruction_bpmn_to_uin/1697719766061.png)

   Если учетные данные верны, в результате выполнения команды в выбранной рабочей папке появится папка с названием вашего проекта, внутри которой будет лежать скрытая системная папка `.git`, собирающая информацию о версиях и изменениях, а также все файлы из репозитория. В нашем случае это только файл `template.bpmn`.

   ![1697719877453](image/Instruction_bpmn_to_uin/1697719877453.png)

#### Открытие клонированной папки форка в VSC

Следующим шагом, нужно открыть данную папку в VSC и создать отдельную директорию в этой папке, чтобы разместить там копию шаблона, в котором вы будете работать, как это делалось при [первичных настройках VSC](#настройка-vsc-для-работы-с-bpmnio). После размещения копии и её открытия все готово к работе. Не забывайте, что можно также размещать 2 окна с графическим интерфейсом и текстовым представлением (в коде).

![1697720317254](image/Instruction_bpmn_to_uin/1697720317254.png)

### Сохранение данных в репозитории форка на GitFlic

После того, как модель бизнес-процесса разработана, необходимо передать её в общий репозиторий. Однако для этого, сначала надо обновить форк, в котором ведётся работа, т.к. именно из него будет производиться запрос на слияние.

Общий процесс выгрузки заключается в трех основных действиях: добавление (стейджинг), фиксация изменения (commit или коммит) и передача в удаленный репозиторий (push или пуш). По умолчанию данные действия совершаются из командной строки. Но при использовании VSC можно использовать встроенные инструменты для автоматической отправки. В рамках данного раздела инструкции рассмотрены оба способа, на случай, если вы не будете использовать VSC как основной инструмент для разработки.

#### Выгрузка в репозиторий с использованием командной строки

Предположим, что вы внесли изменения в свой процесс и сохранили файл. Вы готовы передавать его на рассмотрение и слияние.

![1697721062214](image/Instruction_bpmn_to_uin/1697721062214.png)

Для того, чтобы выгрузить данные в удаленный репозиторий с использованием командной строки, вам необходимо выполнить следующие шаги:
...
2. Введите команду
   ...
   чтобы проверить, что у вас есть не добавленные данные (не вошедшие в стейджинг). Отмеченные красным строки указывают на наличие измененных файлов, но не вошедших в стейджинг.

   ![1697721481056](image/Instruction_bpmn_to_uin/1697721481056.png)
3. Чтобы добавить все измененные файлы в стейджинг, введите команду

   ```
   git add .
   ```

   а затем снова проверьте статус добавления. Все добавленные файлы будут зелеными. Красных больше быть не должно. Это значит, что данные готовы к фиксации (комиту).

   > Вместо точки можно вводить имена отдельных файлов и папок, если вдруг вы что то изменили но добавить хотите что-то отдельное
   >

   ![1697721705503](image/Instruction_bpmn_to_uin/1697721705503.png)
4. Следующим шагом нужно сделать фиксацию версии (комит). Для этого необходимо ввести команду git commit -m "Текст комментария" после чего Git сохранит данный комит на вашем компьютере. Команда -m позволяет добавить комментарий, который будет отражать что добавлено в этот комит (например, причина изменения файлов).

   ![1697722271072](image/Instruction_bpmn_to_uin/1697722271072.png)
5. Последним, но не менее важным шагом здесь является выгрузка в удаленный репозиторий (пуш). Для того, чтобы отправить данные в репозитория (а это, помним, пока еще копия основного), нужно набрать команду
...
Предположим, что снова были внесены изменения в модель бизнес-процесса. При VSC открыт и в нем открыта папка клонированного проекта. Если так, то в левой панели инструментов, в разделе Explorer напротив измененного файла появится индекс "М", а при наведении в подсказке помимо пути к файлу будет указан статус Modified (1). Это аналогично тому, что происходит при работе с командной строкой, когда по команде git status программа красным отмечает изменённые, но не добавленные файлы. Кроме того, на той же панели есть раздел "Source Control", у которого появился индекс "1", отражающий наличие изменений (2).
...
В открывшемся разделе будут отражены все ранее сделанные коммиты. В нашем случае - две версии, выгруженные сначала [через командную строку](#выгрузка-в-репозиторий-с-использованием-командной-строки), а затем [с инструментами VSC](#выгрузка-в-репозиторий-с-использованием-инструментов-vsc).
...
Теперь необходимо направить на рассмотрение результаты выполненной работы. Для этого используется инструмент запросов на слияния (Pull Request), который, при согласовании куратором проекта, автоматически размещает результаты в основном репозитории проекта (т.е. сливает основной с форком).
...
### Обновление данных форка из исходного репозитория

Предположим, что в исходный шаблон куратор проекта внес изменения и выгрузил в основной репозиторий (как он выглядел изначально смотреть в разделе, где [открывалась клонированная папка](#открытие-клонированной-папки-форка-в-vsc)). Допустим, добавились новые дорожки и дополнительный пул. Та же ситуация будет, если куратор одобрил слияние с форков других участников.

![1697732353625](image/Instruction_bpmn_to_uin/1697732353625.png)

Если это произойдет, в форке репозитория разработчика процессов появится фраза о том, что "Эта ветка отстает на Х коммитов" и появится команда "Синхронизировать форк" (1). При нажатии на эту команду появится всплывающее окно, предупреждающее, что при синхронизации могут возникнуть конфликты, поскольку вы уже вносили изменения, что соответственно приведет к замене сделанных вами изменений в тех же файлах, которые были обновлены в исходном репозитории. Но поскольку в нашем случае менялся только файл исходного шаблона, наши изменения сохранятся. При нажатии "Обновить ветку" (2) произойдет обновление **удалённого** репозитория (т.е. с него надо будет подтянуть эти изменения на локальный компьютер).
...
```
 ...
 либо, если вы работаете в VSC, выберете раздел "Source Control", и в выпадающем меню "..." выберете команду Pull.
